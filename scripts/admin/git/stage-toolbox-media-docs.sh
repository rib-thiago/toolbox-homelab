...
# Stage Toolbox media documentation.
#
# This script stages only the approved docs/media Stockhausen documentation files.
# It does not commit.
# It does not stage scripts/media, Stockhausen scripts, generated outputs,
...
#
# Fallback mode:
#   stage-toolbox-media-docs.sh APPLY
...
REPORT_FILE="$(toolbox_report_path "toolbox_media_docs" "stage" "$STAMP")"
TSV_FILE="$(toolbox_tsv_path "toolbox_media_docs" "stage" "$STAMP")"
...
write_headers() {
  {
    printf 'Toolbox media documentation staging report\n'
    ...
    printf '\n'
    printf 'Scope:\n'
    printf '  stage only Stockhausen media documentation under docs/media;\n'
    printf '  no commit;\n'
    printf '  no git add .;\n'
    printf '  no scripts/media staged;\n'
...
git_required() {
  ...
  if git -C "$TOOLBOX_APP" rev-parse --show-toplevel >/tmp/toolbox-stage-media-docs-root.$$ 2>/tmp/toolbox-stage-media-docs-err.$$; then
    local root
    local branch

    root="$(cat /tmp/toolbox-stage-media-docs-root.$$)"
    ...
    local err
    err="$(cat /tmp/toolbox-stage-media-docs-err.$$ 2>/dev/null || printf 'unknown error')"
    record "git" "$TOOLBOX_APP" "repository" "fail" "$err"
    rm -f /tmp/toolbox-stage-media-docs-root.$$ /tmp/toolbox-stage-media-docs-err.$$
...
show_plan() {
  ...
  {
    printf 'The following media documentation files will be staged:\n'
    printf '\n'
  } >> "$REPORT_FILE"
  ...
  for path in $DOCS_TO_STAGE; do
    printf '  %s\n' "$path"
    printf '  %s\n' "$path" >> "$REPORT_FILE"
    record "plan" "$path" "stage candidate" "planned" "Stockhausen media documentation"
  done

  printf '\n'
  printf 'It will NOT commit.\n'
  printf 'It will NOT stage scripts/media, operational docs, reports, TSVs, snapshots, .save files, or generated outputs.\n'
  printf '\n'
}

validate_expected_files() {
  section "Validate expected docs/media files"
...
validate_no_backup_files_in_scope() {
  section "Validate no editor backups in docs/media scope"
  ...
  } >/tmp/toolbox-stage-media-docs-backups.$$

  while IFS= read -r path; do
    [ -n "$path" ] || continue
    found="yes"
    record "exclude" "$path" "editor backup detected" "warning" "will not be staged by this script"
  done < /tmp/toolbox-stage-media-docs-backups.$$

  rm -f /tmp/toolbox-stage-media-docs-backups.$$

  if [ "$found" = "no" ]; then
    record "exclude" "docs/media editor backups" "scan" "ok" "no editor backup files found in docs/media"
  fi
}

write_pre_status() {
  section "Pre-stage Git status for selected docs/media files"
  ...
  record "git" "selected docs/media" "pre-stage status" "recorded" "see report"
}
...
require_apply_confirmation() {
  ...
  printf 'About to stage Stockhausen media documentation files listed above.\n'
...
stage_docs() {
  section "Stage selected docs/media files"
...
write_cached_diff_summary() {
...

  {
    ...
    printf '\n'
    printf 'git status --short for selected docs/media files:\n'
    for path in $DOCS_TO_STAGE; do
      git status --short -- "$path"
    done
...
validate_only_expected_docs_staged() {
...

  {
    cd "$TOOLBOX_APP" || exit 0
    git diff --cached --name-only
  } >/tmp/toolbox-stage-media-docs-cached-files.$$
  ...
  while IFS= read -r staged; do
    ...
    if [ "$ok" = "yes" ]; then
      ...
      unexpected="yes"
      record "staged-set" "$staged" "unexpected staged file" "fail" "not part of docs/media staging plan"
    fi
  done < /tmp/toolbox-stage-media-docs-cached-files.$$

  rm -f /tmp/toolbox-stage-media-docs-cached-files.$$
...
write_next_commands() {
...

  {
    ...
    printf '  git diff --cached --stat\n'
    printf '  git diff --cached -- docs/media\n'
    printf '\n'
    printf 'If everything is correct, commit manually:\n'
    printf '\n'
    printf '  git commit -m "docs/media: add stockhausen media documentation"\n'
    printf '\n'
    printf 'If something is wrong, unstage these files:\n'
    printf '\n'
    printf '  git restore --staged \\\n'
    printf '    docs/media/stockhausen_gold_model_stimmung.md \\\n'
    printf '    docs/media/stockhausen_metadata_policy.md\n'
  } >> "$REPORT_FILE"
...
write_summary() {
  ...
  if [ "$fail_count" -eq 0 ]; then
    record "summary" "media-docs-stage" "failed checks" "ok" "fail_count=0"
  else
    record "summary" "media-docs-stage" "failed checks" "fail" "fail_count=${fail_count}"
  fi
}
...
main() {
  ...
  log "Preparing to stage Toolbox media documentation."
  ...
  log "Media documentation staging completed."
  ...
  printf '  TSV:          %s\n' "$TSV_FILE"
  printf '\n'
  printf 'Review staged docs/media with:\n'
  printf '  cd /srv/toolbox/app || exit 1\n'
  printf '  git diff --cached --stat\n'
  printf '  git diff --cached -- docs/media\n'
}
