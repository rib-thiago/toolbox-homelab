...
# Stage Stockhausen album 050 import workflow.
#
# This script stages only the approved Stockhausen album 050 import scripts.
#
# It does not commit.
...
#
# Fallback mode:
#   stage-toolbox-stockhausen-import-050.sh APPLY
...
REPORT_FILE="$(toolbox_report_path "toolbox_stockhausen_import_050" "stage" "$STAMP")"
TSV_FILE="$(toolbox_tsv_path "toolbox_stockhausen_import_050" "stage" "$STAMP")"

mkdir -p "$(dirname "$REPORT_FILE")"
mkdir -p "$(dirname "$TSV_FILE")"

SCRIPTS_TO_STAGE="
scripts/media/stockhausen/diagnose-stockhausen-import-050.sh
scripts/media/stockhausen/analyze-stockhausen-import-050-splits.sh
scripts/media/stockhausen/split-stockhausen-import-050.sh
scripts/media/stockhausen/plan-stockhausen-import-050.sh
scripts/media/stockhausen/apply-stockhausen-import-050.sh
scripts/media/stockhausen/validate-stockhausen-import-050.sh
"
...
write_headers() {
  {
    printf 'Stockhausen album 050 import workflow staging report\n'
    ...
    printf '\n'
    printf 'Scope:\n'
    printf '  stage only Stockhausen album 050 import workflow scripts;\n'
    printf '  no commit;\n'
    printf '  no git add .;\n'
...
git_required() {
  ...
  if git -C "$TOOLBOX_APP" rev-parse --show-toplevel >/tmp/toolbox-stage-stockhausen-import-050-root.$$ 2>/tmp/toolbox-stage-stockhausen-import-050-err.$$; then
    local root
    local branch

    root="$(cat /tmp/toolbox-stage-stockhausen-import-050-root.$$)"
    ...
    local err
    err="$(cat /tmp/toolbox-stage-stockhausen-import-050-err.$$ 2>/dev/null || printf 'unknown error')"
    record "git" "$TOOLBOX_APP" "repository" "fail" "$err"
    rm -f /tmp/toolbox-stage-stockhausen-import-050-root.$$ /tmp/toolbox-stage-stockhausen-import-050-err.$$
...
show_plan() {
  ...
  {
    printf 'The following Stockhausen album 050 import scripts will be staged:\n'
    printf '\n'
  } >> "$REPORT_FILE"
  ...
  for path in $SCRIPTS_TO_STAGE; do
    printf '  %s\n' "$path"
    printf '  %s\n' "$path" >> "$REPORT_FILE"
    record "plan" "$path" "stage candidate" "planned" "Stockhausen album 050 import workflow"
  done

  printf '\n'
  printf 'It will NOT commit.\n'
  printf 'It will NOT stage diagnostics/gold, normalization, artwork, final-freeze, reports, TSVs, snapshots, .save files, or generated outputs.\n'
  printf '\n'
}

validate_expected_files() {
  section "Validate expected Stockhausen import 050 scripts"
...
validate_bashcheck() {
  ...
  for path in $SCRIPTS_TO_STAGE; do
    ...
    if bash -n "$full_path" >/tmp/toolbox-stage-stockhausen-import-050-bashcheck-stdout.$$ 2>/tmp/toolbox-stage-stockhausen-import-050-bashcheck-stderr.$$; then
      record "bashcheck" "$path" "bash -n" "ok" "syntax ok"
    else
      local err
      err="$(cat /tmp/toolbox-stage-stockhausen-import-050-bashcheck-stderr.$$ 2>/dev/null || printf 'unknown error')"
      record "bashcheck" "$path" "bash -n" "fail" "$err"
      rm -f /tmp/toolbox-stage-stockhausen-import-050-bashcheck-stdout.$$ /tmp/toolbox-stage-stockhausen-import-050-bashcheck-stderr.$$
...
validate_no_backup_files_in_scope() {
  ...
  } >/tmp/toolbox-stage-stockhausen-import-050-backups.$$

  while IFS= read -r path; do
    [ -n "$path" ] || continue
    found="yes"
    record "exclude" "$path" "editor backup detected" "warning" "will not be staged by this script"
  done < /tmp/toolbox-stage-stockhausen-import-050-backups.$$

  rm -f /tmp/toolbox-stage-stockhausen-import-050-backups.$$
...
write_pre_status() {
  section "Pre-stage Git status for selected Stockhausen import 050 scripts"
  ...
  record "git" "selected Stockhausen import 050 scripts" "pre-stage status" "recorded" "see report"
}
...
require_apply_confirmation() {
  ...
  printf 'About to stage Stockhausen album 050 import workflow scripts listed above.\n'
...
stage_scripts() {
  section "Stage selected Stockhausen import 050 scripts"
...
write_cached_diff_summary() {
...

  {
    ...
    printf '\n'
    printf 'git status --short for selected Stockhausen import 050 scripts:\n'
    for path in $SCRIPTS_TO_STAGE; do
      git status --short -- "$path"
    done
...
validate_only_expected_scripts_staged() {
...

  {
    cd "$TOOLBOX_APP" || exit 0
    git diff --cached --name-only
  } >/tmp/toolbox-stage-stockhausen-import-050-cached-files.$$
  ...
  while IFS= read -r staged; do
    ...
    if [ "$ok" = "yes" ]; then
      ...
      unexpected="yes"
      record "staged-set" "$staged" "unexpected staged file" "fail" "not part of Stockhausen import 050 staging plan"
    fi
  done < /tmp/toolbox-stage-stockhausen-import-050-cached-files.$$

  rm -f /tmp/toolbox-stage-stockhausen-import-050-cached-files.$$
...
write_next_commands() {
...

  {
    ...
    printf '\n'
    printf '  git commit -m "scripts/media: add stockhausen album 050 import workflow"\n'
    printf '\n'
...
write_summary() {
  ...
  if [ "$fail_count" -eq 0 ]; then
    record "summary" "stockhausen-import-050-stage" "failed checks" "ok" "fail_count=0"
  else
    record "summary" "stockhausen-import-050-stage" "failed checks" "fail" "fail_count=${fail_count}"
  fi
}
...
main() {
  ...
  log "Preparing to stage Stockhausen album 050 import workflow scripts."
  ...
  log "Stockhausen album 050 import workflow staging completed."
  ...
  printf '  TSV:          %s\n' "$TSV_FILE"
  printf '\n'
  printf 'Review staged Stockhausen import 050 scripts with:\n'
  printf '  cd /srv/toolbox/app || exit 1\n'
  printf '  git diff --cached --stat\n'
  printf '  git diff --cached -- scripts/media/stockhausen\n'
}
