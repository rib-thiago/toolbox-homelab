...
# Stage Toolbox media library and Soulseek tooling.
#
# This script stages only the approved scripts/media/library and
# scripts/media/soulseek scripts.
#
# It does not commit.
# It does not stage Stockhausen scripts, docs/media, generated outputs,
# editor backups, operational docs, or unrelated files.
#
# Preferred mode:
#   run script, review plan, type APPLY interactively.
#
# Fallback mode:
#   stage-toolbox-media-library-soulseek.sh APPLY
...
REPORT_FILE="$(toolbox_report_path "toolbox_media_library_soulseek" "stage" "$STAMP")"
TSV_FILE="$(toolbox_tsv_path "toolbox_media_library_soulseek" "stage" "$STAMP")"

mkdir -p "$(dirname "$REPORT_FILE")"
mkdir -p "$(dirname "$TSV_FILE")"

SCRIPTS_TO_STAGE="
scripts/media/library/build-music-inventory-tsv.sh
scripts/media/library/init-music-library-sqlite.sh
scripts/media/library/scan-music-library.sh
scripts/media/soulseek/reorganize-music-staging.sh
"
...
write_headers() {
  {
    printf 'Toolbox media library/Soulseek staging report\n'
    ...
    printf '\n'
    printf 'Scope:\n'
    printf '  stage only scripts/media/library and scripts/media/soulseek tooling;\n'
    printf '  no commit;\n'
    printf '  no git add .;\n'
    printf '  no Stockhausen scripts staged;\n'
    printf '  no docs/media staged;\n'
...
git_required() {
  ...
  if git -C "$TOOLBOX_APP" rev-parse --show-toplevel >/tmp/toolbox-stage-media-lib-root.$$ 2>/tmp/toolbox-stage-media-lib-err.$$; then
    local root
    local branch

    root="$(cat /tmp/toolbox-stage-media-lib-root.$$)"
    ...
    local err
    err="$(cat /tmp/toolbox-stage-media-lib-err.$$ 2>/dev/null || printf 'unknown error')"
    record "git" "$TOOLBOX_APP" "repository" "fail" "$err"
    rm -f /tmp/toolbox-stage-media-lib-root.$$ /tmp/toolbox-stage-media-lib-err.$$
...
show_plan() {
  ...
  {
    printf 'The following media library/Soulseek scripts will be staged:\n'
    printf '\n'
  } >> "$REPORT_FILE"
  ...
  for path in $SCRIPTS_TO_STAGE; do
    printf '  %s\n' "$path"
    printf '  %s\n' "$path" >> "$REPORT_FILE"
    record "plan" "$path" "stage candidate" "planned" "media library/Soulseek tooling"
  done

  printf '\n'
  printf 'It will NOT commit.\n'
  printf 'It will NOT stage Stockhausen scripts, docs/media, reports, TSVs, snapshots, .save files, or generated outputs.\n'
  printf '\n'
}

validate_expected_files() {
  section "Validate expected media library/Soulseek scripts"
...
validate_bashcheck() {
  ...
  for path in $SCRIPTS_TO_STAGE; do
    ...
    if bash -n "$full_path" >/tmp/toolbox-stage-media-lib-bashcheck-stdout.$$ 2>/tmp/toolbox-stage-media-lib-bashcheck-stderr.$$; then
      record "bashcheck" "$path" "bash -n" "ok" "syntax ok"
    else
      local err
      err="$(cat /tmp/toolbox-stage-media-lib-bashcheck-stderr.$$ 2>/dev/null || printf 'unknown error')"
      record "bashcheck" "$path" "bash -n" "fail" "$err"
      rm -f /tmp/toolbox-stage-media-lib-bashcheck-stdout.$$ /tmp/toolbox-stage-media-lib-bashcheck-stderr.$$
...
validate_no_backup_files_in_scope() {
  section "Validate no editor backups in media library/Soulseek scope"
...

  {
    ...
    find scripts/media/library scripts/media/soulseek \
      \( -name '*.save' -o -name '*.save.*' -o -name '*.bak' -o -name '*~' -o -name '.#*' \) \
      -print 2>/dev/null | sort
  } >/tmp/toolbox-stage-media-lib-backups.$$

  while IFS= read -r path; do
    [ -n "$path" ] || continue
    found="yes"
    record "exclude" "$path" "editor backup detected" "warning" "will not be staged by this script"
  done < /tmp/toolbox-stage-media-lib-backups.$$

  rm -f /tmp/toolbox-stage-media-lib-backups.$$

  if [ "$found" = "no" ]; then
    record "exclude" "media library/Soulseek editor backups" "scan" "ok" "no editor backup files found"
  fi
}

write_pre_status() {
  section "Pre-stage Git status for selected media scripts"
  ...
  record "git" "selected media scripts" "pre-stage status" "recorded" "see report"
}
...
require_apply_confirmation() {
  ...
  printf 'About to stage media library/Soulseek scripts listed above.\n'
...
stage_scripts() {
  section "Stage selected media library/Soulseek scripts"
...
write_cached_diff_summary() {
...

  {
    ...
    printf '\n'
    printf 'git status --short for selected media scripts:\n'
    for path in $SCRIPTS_TO_STAGE; do
      git status --short -- "$path"
    done
...
validate_only_expected_scripts_staged() {
...

  {
    cd "$TOOLBOX_APP" || exit 0
    git diff --cached --name-only
  } >/tmp/toolbox-stage-media-lib-cached-files.$$
  ...
  while IFS= read -r staged; do
    ...
    if [ "$ok" = "yes" ]; then
      ...
      unexpected="yes"
      record "staged-set" "$staged" "unexpected staged file" "fail" "not part of media library/Soulseek staging plan"
    fi
  done < /tmp/toolbox-stage-media-lib-cached-files.$$

  rm -f /tmp/toolbox-stage-media-lib-cached-files.$$
...
write_next_commands() {
...

  {
    ...
    printf '  git diff --cached --stat\n'
    printf '  git diff --cached -- scripts/media/library scripts/media/soulseek\n'
    printf '\n'
    printf 'If everything is correct, commit manually:\n'
    printf '\n'
    printf '  git commit -m "scripts/media: add library and soulseek tooling"\n'
    printf '\n'
    printf 'If something is wrong, unstage these files:\n'
    printf '\n'
    printf '  git restore --staged \\\n'
    printf '    scripts/media/library/build-music-inventory-tsv.sh \\\n'
    printf '    scripts/media/library/init-music-library-sqlite.sh \\\n'
    printf '    scripts/media/library/scan-music-library.sh \\\n'
    printf '    scripts/media/soulseek/reorganize-music-staging.sh\n'
  } >> "$REPORT_FILE"
...
write_summary() {
  ...
  if [ "$fail_count" -eq 0 ]; then
    record "summary" "media-library-soulseek-stage" "failed checks" "ok" "fail_count=0"
  else
    record "summary" "media-library-soulseek-stage" "failed checks" "fail" "fail_count=${fail_count}"
  fi
}
...
main() {
  ...
  log "Preparing to stage Toolbox media library/Soulseek scripts."
  ...
  log "Media library/Soulseek staging completed."
  ...
  printf '  TSV:          %s\n' "$TSV_FILE"
  printf '\n'
  printf 'Review staged media library/Soulseek scripts with:\n'
  printf '  cd /srv/toolbox/app || exit 1\n'
  printf '  git diff --cached --stat\n'
  printf '  git diff --cached -- scripts/media/library scripts/media/soulseek\n'
}
