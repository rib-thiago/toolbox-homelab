...
# Stage Toolbox Phase 2 admin diagnostic/validation/staging scripts.
#
# This script stages only the approved Phase 2 admin scripts.
# It does not commit.
# It does not stage tbman workflow scripts, media scripts, Stockhausen scripts,
# generated outputs, editor backups, docs/media, or unrelated scripts.
#
# Preferred mode:
#   run script, review plan, type APPLY interactively.
#
# Fallback mode:
#   stage-toolbox-phase2-admin-scripts.sh APPLY
...
REPORT_FILE="$(toolbox_report_path "toolbox_phase2_admin_scripts" "stage" "$STAMP")"
TSV_FILE="$(toolbox_tsv_path "toolbox_phase2_admin_scripts" "stage" "$STAMP")"

mkdir -p "$(dirname "$REPORT_FILE")"
mkdir -p "$(dirname "$TSV_FILE")"

SCRIPTS_TO_STAGE="
scripts/admin/system/backup-toolbox-phase2-docs.sh
scripts/admin/system/diagnose-toolbox-ergonomics-and-outputs.sh
scripts/admin/system/diagnose-toolbox-output-policy-and-lib.sh
scripts/admin/system/diagnose-toolbox-operations-docs-policy.sh
scripts/admin/system/diagnose-toolbox-architecture-vs-practice.sh
scripts/admin/system/diagnose-toolbox-host-container-tools.sh
scripts/admin/system/validate-toolbox-phase2-docs.sh
scripts/admin/system/validate-toolbox-phase2-policy-docs.sh
scripts/admin/system/validate-toolbox-phase2-documentation-global.sh
scripts/admin/system/validate-toolbox-scripts-lib.sh
scripts/admin/git/plan-toolbox-phase2-git-commit-groups.sh
scripts/admin/git/stage-toolbox-phase2-docs.sh
scripts/admin/git/stage-toolbox-scripts-lib.sh
"
...
write_headers() {
  {
    printf 'Toolbox Phase 2 admin scripts staging report\n'
    ...
    printf '\n'
    printf 'Scope:\n'
    printf '  stage only Phase 2 diagnostic/validation/staging admin scripts;\n'
    printf '  no commit;\n'
    printf '  no git add .;\n'
    printf '  no tbman apply/diagnostic scripts staged;\n'
...
git_required() {
  ...
  if git -C "$TOOLBOX_APP" rev-parse --show-toplevel >/tmp/toolbox-stage-admin-git-root.$$ 2>/tmp/toolbox-stage-admin-git-err.$$; then
    local root
    local branch

    root="$(cat /tmp/toolbox-stage-admin-git-root.$$)"
    ...
    local err
    err="$(cat /tmp/toolbox-stage-admin-git-err.$$ 2>/dev/null || printf 'unknown error')"
    record "git" "$TOOLBOX_APP" "repository" "fail" "$err"
    rm -f /tmp/toolbox-stage-admin-git-root.$$ /tmp/toolbox-stage-admin-git-err.$$
...
show_plan() {
  ...
  {
    printf 'The following admin scripts will be staged:\n'
    printf '\n'
  } >> "$REPORT_FILE"
  ...
  for path in $SCRIPTS_TO_STAGE; do
    printf '  %s\n' "$path"
    printf '  %s\n' "$path" >> "$REPORT_FILE"
    record "plan" "$path" "stage candidate" "planned" "Phase 2 admin diagnostic/validation/staging script"
  done

  printf '\n'
  printf 'It will NOT commit.\n'
  printf 'It will NOT stage tbman workflow scripts, media scripts, Stockhausen scripts, editor backups, generated outputs, or unrelated scripts.\n'
  printf '\n'

  {
    ...
    for path in $EXCLUDED_KNOWN; do
      printf '  %s\n' "$path"
      record "exclude" "$path" "known excluded from this group" "planned" "separate review/commit group"
    done
  } >> "$REPORT_FILE"
}
...
validate_bashcheck() {
  section "Validate bash syntax for staged admin scripts"
  ...
  for path in $SCRIPTS_TO_STAGE; do
    ...
    if bash -n "$full_path" >/tmp/toolbox-stage-admin-bashcheck-stdout.$$ 2>/tmp/toolbox-stage-admin-bashcheck-stderr.$$; then
      record "bashcheck" "$path" "bash -n" "ok" "syntax ok"
    else
      local err
      err="$(cat /tmp/toolbox-stage-admin-bashcheck-stderr.$$ 2>/dev/null || printf 'unknown error')"
      record "bashcheck" "$path" "bash -n" "fail" "$err"
      rm -f /tmp/toolbox-stage-admin-bashcheck-stdout.$$ /tmp/toolbox-stage-admin-bashcheck-stderr.$$
...
validate_no_backup_files_in_scope() {
  section "Validate no editor backups in admin staging scope"
  ...
  } >/tmp/toolbox-stage-admin-save-files.$$

  while IFS= read -r path; do
    [ -n "$path" ] || continue
    found="yes"
    record "exclude" "$path" "editor backup detected" "warning" "will not be staged by this script"
  done < /tmp/toolbox-stage-admin-save-files.$$

  rm -f /tmp/toolbox-stage-admin-save-files.$$

  if [ "$found" = "no" ]; then
    record "exclude" "scripts/admin editor backups" "scan" "ok" "no editor backup files found in scripts/admin"
  fi
}

write_pre_status() {
  section "Pre-stage Git status for admin scripts"
  ...
  record "git" "admin scripts" "pre-stage status" "recorded" "see report"
}
...
run_validation_before_stage() {
  ...
  if command -v validate-toolbox-phase2-documentation-global.sh >/dev/null 2>&1; then
    if validate-toolbox-phase2-documentation-global.sh >/tmp/toolbox-stage-admin-doc-validation.$$ 2>&1; then
      record "validation" "validate-toolbox-phase2-documentation-global.sh" "pre-stage" "ok" "global documentation validation passed"
    else
      local output
      output="$(cat /tmp/toolbox-stage-admin-doc-validation.$$ 2>/dev/null || printf 'unknown validation error')"
      record "validation" "validate-toolbox-phase2-documentation-global.sh" "pre-stage" "fail" "$output"
      rm -f /tmp/toolbox-stage-admin-doc-validation.$$
      fail "Global documentation validation failed."
    fi

    {
      printf '\nDocumentation validation command output:\n'
      cat /tmp/toolbox-stage-admin-doc-validation.$$ 2>/dev/null || true
    } >> "$REPORT_FILE"
  ...
  if command -v validate-toolbox-scripts-lib.sh >/dev/null 2>&1; then
    if validate-toolbox-scripts-lib.sh >/tmp/toolbox-stage-admin-lib-validation.$$ 2>&1; then
      record "validation" "validate-toolbox-scripts-lib.sh" "pre-stage" "ok" "scripts/lib validation passed"
    else
      local output
      output="$(cat /tmp/toolbox-stage-admin-lib-validation.$$ 2>/dev/null || printf 'unknown validation error')"
      record "validation" "validate-toolbox-scripts-lib.sh" "pre-stage" "fail" "$output"
      rm -f /tmp/toolbox-stage-admin-lib-validation.$$
      fail "scripts/lib validation failed."
    fi

    {
      printf '\nscripts/lib validation command output:\n'
      cat /tmp/toolbox-stage-admin-lib-validation.$$ 2>/dev/null || true
    } >> "$REPORT_FILE"
...
require_apply_confirmation() {
  ...
  printf 'About to stage Phase 2 admin diagnostic/validation scripts listed above.\n'
...
stage_admin_scripts() {
  section "Stage selected admin scripts"
...
write_cached_diff_summary() {
...

  {
    ...
    printf '\n'
    printf 'git status --short for selected admin scripts:\n'
    for path in $SCRIPTS_TO_STAGE; do
      git status --short -- "$path"
    done
...
validate_only_expected_admin_files_staged() {
...

  {
    cd "$TOOLBOX_APP" || exit 0
    git diff --cached --name-only
  } >/tmp/toolbox-stage-admin-cached-files.$$
  ...
  while IFS= read -r staged; do
    ...
    if [ "$ok" = "yes" ]; then
      ...
      unexpected="yes"
      record "staged-set" "$staged" "unexpected staged file" "fail" "not part of Phase 2 admin scripts staging plan"
    fi
  done < /tmp/toolbox-stage-admin-cached-files.$$

  rm -f /tmp/toolbox-stage-admin-cached-files.$$
...
write_next_commands() {
...

  {
    ...
    printf '\n'
    printf '  git commit -m "scripts/admin: add toolbox phase 2 diagnostics and validators"\n'
    printf '\n'
...
write_summary() {
  ...
  if [ "$fail_count" -eq 0 ]; then
    record "summary" "phase2-admin-scripts-stage" "failed checks" "ok" "fail_count=0"
  else
    record "summary" "phase2-admin-scripts-stage" "failed checks" "fail" "fail_count=${fail_count}"
  fi
}
...
main() {
  ...
  log "Preparing to stage Toolbox Phase 2 admin scripts."
  ...
  log "Phase 2 admin scripts staging completed."
  ...
  printf '  TSV:          %s\n' "$TSV_FILE"
  printf '\n'
  printf 'Review staged admin scripts with:\n'
  printf '  cd /srv/toolbox/app || exit 1\n'
  printf '  git diff --cached --stat\n'
  printf '  git diff --cached -- scripts/admin/system scripts/admin/git\n'
}
