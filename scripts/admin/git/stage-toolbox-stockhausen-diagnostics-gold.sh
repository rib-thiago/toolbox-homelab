...
# Stage Stockhausen diagnostics and gold model tooling.
#
# This script stages only the approved Stockhausen diagnostic, audit,
# selection and gold-model scripts.
#
# It does not commit.
# It does not stage normalization, artwork, album 050 import, final freeze,
# Navidrome, generated outputs, editor backups, operational docs, or unrelated files.
#
# Preferred mode:
#   run script, review plan, type APPLY interactively.
#
# Fallback mode:
#   stage-toolbox-stockhausen-diagnostics-gold.sh APPLY
...
REPORT_FILE="$(toolbox_report_path "toolbox_stockhausen_diagnostics_gold" "stage" "$STAMP")"
TSV_FILE="$(toolbox_tsv_path "toolbox_stockhausen_diagnostics_gold" "stage" "$STAMP")"

mkdir -p "$(dirname "$REPORT_FILE")"
mkdir -p "$(dirname "$TSV_FILE")"

SCRIPTS_TO_STAGE="
scripts/media/stockhausen/analyze-stockhausen-catalog-reconciliation-1977-95.sh
scripts/media/stockhausen/analyze-stockhausen-library-quality.sh
scripts/media/stockhausen/analyze-stockhausen-medium-albums-1977-95.sh
scripts/media/stockhausen/analyze-stockhausen-partials.sh
scripts/media/stockhausen/scan-stockhausen-metadata-patterns.sh
scripts/media/stockhausen/diagnose-stockhausen-gold-model-candidates.sh
scripts/media/stockhausen/extract-stockhausen-gold-model.sh
scripts/media/stockhausen/extract-stockhausen-reference-album.sh
scripts/media/stockhausen/plan-stockhausen-gold-normalization.sh
scripts/media/stockhausen/apply-stockhausen-gold-normalization.sh
scripts/media/stockhausen/validate-stockhausen-gold-normalization.sh
scripts/media/stockhausen/filter-stockhausen-plan-high-only.sh
scripts/media/stockhausen/stockhausen-verlag-audit.sh
"
...
write_headers() {
  {
    printf 'Stockhausen diagnostics/gold model staging report\n'
    ...
    printf '\n'
    printf 'Scope:\n'
    printf '  stage only Stockhausen diagnostics, audit, selection and gold model tooling;\n'
    printf '  no commit;\n'
    printf '  no git add .;\n'
    printf '  no normalization batch scripts staged;\n'
    printf '  no artwork workflow scripts staged;\n'
    printf '  no album 050 import scripts staged;\n'
    printf '  no final freeze/Navidrome scripts staged;\n'
...
git_required() {
  ...
  if git -C "$TOOLBOX_APP" rev-parse --show-toplevel >/tmp/toolbox-stage-stockhausen-gold-root.$$ 2>/tmp/toolbox-stage-stockhausen-gold-err.$$; then
    local root
    local branch

    root="$(cat /tmp/toolbox-stage-stockhausen-gold-root.$$)"
    ...
    local err
    err="$(cat /tmp/toolbox-stage-stockhausen-gold-err.$$ 2>/dev/null || printf 'unknown error')"
    record "git" "$TOOLBOX_APP" "repository" "fail" "$err"
    rm -f /tmp/toolbox-stage-stockhausen-gold-root.$$ /tmp/toolbox-stage-stockhausen-gold-err.$$
...
show_plan() {
  ...
  {
    printf 'The following Stockhausen diagnostics/gold model scripts will be staged:\n'
    printf '\n'
  } >> "$REPORT_FILE"
  ...
  for path in $SCRIPTS_TO_STAGE; do
    printf '  %s\n' "$path"
    printf '  %s\n' "$path" >> "$REPORT_FILE"
    record "plan" "$path" "stage candidate" "planned" "Stockhausen diagnostics/gold model tooling"
  done

  printf '\n'
  printf 'It will NOT commit.\n'
  printf 'It will NOT stage normalization, artwork, import-050, final-freeze, reports, TSVs, snapshots, .save files, or generated outputs.\n'
  printf '\n'
}

validate_expected_files() {
  section "Validate expected Stockhausen diagnostics/gold scripts"
...
validate_bashcheck() {
  ...
  for path in $SCRIPTS_TO_STAGE; do
    ...
    if bash -n "$full_path" >/tmp/toolbox-stage-stockhausen-gold-bashcheck-stdout.$$ 2>/tmp/toolbox-stage-stockhausen-gold-bashcheck-stderr.$$; then
      record "bashcheck" "$path" "bash -n" "ok" "syntax ok"
    else
      local err
      err="$(cat /tmp/toolbox-stage-stockhausen-gold-bashcheck-stderr.$$ 2>/dev/null || printf 'unknown error')"
      record "bashcheck" "$path" "bash -n" "fail" "$err"
      rm -f /tmp/toolbox-stage-stockhausen-gold-bashcheck-stdout.$$ /tmp/toolbox-stage-stockhausen-gold-bashcheck-stderr.$$
...
validate_no_backup_files_in_scope() {
  ...
  } >/tmp/toolbox-stage-stockhausen-gold-backups.$$

  while IFS= read -r path; do
    [ -n "$path" ] || continue
    found="yes"
    record "exclude" "$path" "editor backup detected" "warning" "will not be staged by this script"
  done < /tmp/toolbox-stage-stockhausen-gold-backups.$$

  rm -f /tmp/toolbox-stage-stockhausen-gold-backups.$$
...
require_apply_confirmation() {
  ...
  printf 'About to stage Stockhausen diagnostics/gold model scripts listed above.\n'
...
stage_scripts() {
  section "Stage selected Stockhausen diagnostics/gold scripts"
...
validate_only_expected_scripts_staged() {
...

  {
    cd "$TOOLBOX_APP" || exit 0
    git diff --cached --name-only
  } >/tmp/toolbox-stage-stockhausen-gold-cached-files.$$
  ...
  while IFS= read -r staged; do
    ...
    if [ "$ok" = "yes" ]; then
      ...
      unexpected="yes"
      record "staged-set" "$staged" "unexpected staged file" "fail" "not part of Stockhausen diagnostics/gold staging plan"
    fi
  done < /tmp/toolbox-stage-stockhausen-gold-cached-files.$$

  rm -f /tmp/toolbox-stage-stockhausen-gold-cached-files.$$
...
write_next_commands() {
...

  {
    ...
    printf '\n'
    printf '  git commit -m "scripts/media: add stockhausen diagnostics and gold model tooling"\n'
    printf '\n'
...
write_summary() {
  ...
  if [ "$fail_count" -eq 0 ]; then
    record "summary" "stockhausen-diagnostics-gold-stage" "failed checks" "ok" "fail_count=0"
  else
    record "summary" "stockhausen-diagnostics-gold-stage" "failed checks" "fail" "fail_count=${fail_count}"
  fi
}
...
main() {
  ...
  log "Preparing to stage Stockhausen diagnostics/gold model scripts."
  ...
  log "Stockhausen diagnostics/gold model staging completed."
  ...
  printf '  TSV:          %s\n' "$TSV_FILE"
  printf '\n'
  printf 'Review staged Stockhausen diagnostics/gold scripts with:\n'
  printf '  cd /srv/toolbox/app || exit 1\n'
  printf '  git diff --cached --stat\n'
  printf '  git diff --cached -- scripts/media/stockhausen\n'
}
