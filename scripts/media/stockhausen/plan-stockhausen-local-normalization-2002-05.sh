...
PLAN_TSV="$(ls -t /srv/toolbox/shared/library-db/raw/stockhausen_batch_normalization_plan_2002-05_*.tsv 2>/dev/null | head -1)"
RAW_DIR="/srv/toolbox/shared/library-db/raw"
REPORT_DIR="/srv/toolbox/shared/reports/media"
STAMP="$(date +%Y%m%d-%H%M%S)"

OUT_TSV="$RAW_DIR/stockhausen_local_normalization_plan_2002-05_LOCAL_$STAMP.tsv"
REPORT="$REPORT_DIR/stockhausen_local_normalization_plan_2002-05_LOCAL_$STAMP.txt"

[[ -f "$PLAN_TSV" ]] || fail "Plano completo 2002-05 não encontrado."
...
target_prefixes = tuple(f"{i:03d}" for i in range(67, 83))
...
with plan.open("r", encoding="utf-8", newline="") as f:
    ...
    for row in reader:
        ...
        rows.append({
            ...
            "proposed_grouping": "2002-05: Licht pt. 3",
...
with report.open("w", encoding="utf-8") as f:
    f.write("Stockhausen local normalization plan — 2002-05 LOCAL\n")
    ...
    f.write("GROUPING=2002-05: Licht pt. 3\n")
    ...
    f.write("- The skipped LOW row should be handled manually after local normalization.\n")
