...
ROOT="/srv/media/music/Karlheinz Stockhausen/2006-15: KLANG & Text CDs"
GROUPING="2006-15: KLANG & Text CDs"
CORPUS_ID="2006-15"
...
PLAN_TSV="$RAW_DIR/stockhausen_batch_normalization_plan_${CORPUS_ID}_$STAMP.tsv"
ALBUM_TSV="$RAW_DIR/stockhausen_batch_album_summary_${CORPUS_ID}_$STAMP.tsv"
ANOMALY_TSV="$RAW_DIR/stockhausen_batch_anomalies_${CORPUS_ID}_$STAMP.tsv"
REPORT="$REPORT_DIR/stockhausen_batch_normalization_plan_${CORPUS_ID}_$STAMP.txt"
...
while IFS= read -r -d '' album_dir; do
  ...
  for f in "${FLACS[@]}"; do
    ...
    if [[ "$track_base" =~ ^[0-9]+$ && -n "$filename_title" ]]; then
      track_padded="$(printf "%02d" "$((10#$track_base))")"
...
{
  echo "Stockhausen batch normalization plan — ${CORPUS_ID} corpus"
