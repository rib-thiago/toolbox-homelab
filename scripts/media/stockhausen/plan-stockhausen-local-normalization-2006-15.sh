...
PLAN_TSV="$(ls -t /srv/toolbox/shared/library-db/raw/stockhausen_batch_normalization_plan_2006-15_*.tsv 2>/dev/null | head -1)"
RAW_DIR="/srv/toolbox/shared/library-db/raw"
REPORT_DIR="/srv/toolbox/shared/reports/media"
STAMP="$(date +%Y%m%d-%H%M%S)"

OUT_TSV="$RAW_DIR/stockhausen_local_normalization_plan_2006-15_LOCAL_$STAMP.tsv"
REPORT="$REPORT_DIR/stockhausen_local_normalization_plan_2006-15_LOCAL_$STAMP.txt"

[[ -f "$PLAN_TSV" ]] || fail "Plano completo 2006-15 não encontrado."
...
target_prefixes = tuple(f"{i:03d}" for i in range(83, 107))
...
with plan.open("r", encoding="utf-8", newline="") as f:
    ...
    for row in reader:
        ...
        rows.append({
            ...
            "proposed_grouping": "2006-15: KLANG & Text CDs",
...
with report.open("w", encoding="utf-8") as f:
    f.write("Stockhausen local normalization plan — 2006-15 LOCAL\n")
    ...
    f.write("- It excludes LOW rows from application.\n")
    f.write("- The skipped LOW rows should be handled manually after local normalization.\n")
