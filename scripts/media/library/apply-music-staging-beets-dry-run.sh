...
REPORT="$REPORT_DIR/music_staging_beets_dry_run_apply_report_$STAMP.txt"
TSV="$RAW_DIR/music_staging_beets_dry_run_apply_$STAMP.tsv"
LIVE_LOG="$REPORT_DIR/beets_dry_run_${SAFE_ALBUM_NAME}_live_$STAMP.log"
...
confirm_apply() {
  ...
  if [ "$APPLY_MODE" != "--apply" ]; then
    fail "This script runs an interactive Beets dry-run. Re-run with: apply-music-staging-beets-dry-run.sh --apply [album_dir]"
  fi

  printf '%s\n' "This script will run an interactive Beets dry-run:"
  printf '%s\n' "- Album: $ALBUM_DIR"
...
main() {
  ...
  log "Starting Beets dry-run apply."
  ...
  {
    printf '%s\n' '# Music Staging Beets Dry-run Apply'
    printf '\n'
  ...
  if [ ! -f "$BEETS_CONFIG" ]; then
    write_step "PRE-003" "preflight" "$BEETS_CONFIG" "FAIL" "Beets config missing" ""
  else
    write_step "PRE-003" "preflight" "$BEETS_CONFIG" "OK" "Beets config exists" ""
  fi
  ...
  printf '%s\n' "Running Beets dry-run. Interact with beet normally."
  printf '%s\n' "Log: $LIVE_LOG"
  ...
  found_no_match="$(grep -Ei 'No matching release found|No matching release' "$LIVE_LOG" || true)"
  ...
  if [ "$fail_count" -gt 0 ]; then
    log "Beets dry-run apply completed with failures."
    log "Report: $REPORT"
    log "TSV: $TSV"
  ...
  log "Beets dry-run apply completed."
  log "Report: $REPORT"
  log "TSV: $TSV"
  log "Live log: $LIVE_LOG"
}
